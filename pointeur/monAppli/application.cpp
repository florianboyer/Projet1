...
void Application::afficherPerson()
{
	...
	for (int noPerson=0; noPerson<nbPerson;noPerson++) //Boucle parcourant le vecteur Personne
	{
		...
		//vectPerson[noPerson]->display(); //affiche ce qu'il y a à l'adresse de Personne 
	}
	if (nbPerson==0) 
	{
...
{
	Person * personAAjouter; //déclaration de Personne a ajouter
	personAAjouter = new Person;
	personAAjouter->input(); //saisir la personne par l'utilisateur
	//*personAAjouter.input();
